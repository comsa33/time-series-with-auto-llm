...
class TimeSeriesVisualizer(metaclass=Singleton):
    ...
    def plot_residuals(
        ...
        fig.update_yaxes(title_text='Ordered Values', row=2, col=1)
    ...
    def plot_differencing_comparison(
            ...
            title: str = "Differencing Comparison",
            **kwargs
        ) -> go.Figure:
        """
    ...
    def plot_stationarity_comparison(
        ...
        # 각 영역에 텍스트 추가
        fig.add_annotation(x=0.25, y=0.25, text="abnormal", showarrow=False)
        fig.add_annotation(x=0.75, y=0.25, text="level stationary", showarrow=False)
        fig.add_annotation(x=0.25, y=0.75, text="trend stationary", showarrow=False)
        fig.add_annotation(x=0.75, y=0.75, text="stationary", showarrow=False)
        ...
        # 축 레이블과 제목 설정
        fig.update_layout(
            title="Stationarity Test Summary",
            xaxis=dict(
            title="KPSS Test",
    ...
    def plot_change_points(
            ...
            title: str = "Structural Change Point Analysis",
            **kwargs
        ) -> go.Figure:
        """
        Visualize structural change points in time series data.
        
        Args:
            series: Original time series data
            change_points_result: Change point detection result dictionary
            title: Plot title
    ...
    def plot_segment_means(
            ...
            series: pd.Series,
            change_points_result: dict,
            title: str = "Segment Means",
            **kwargs
        ) -> go.Figure:
        """
        Visualize mean values for each segment separated by change points.
        ...
        # Horizontal line for mean value of each segment
    ...
    def plot_granger_causality(
        ...
        # 레이아웃 설정
        fig.update_layout(
            title=f'Granger Causality p-values from {cause_var} to {effect_var}',
            xaxis_title='Lag',
            yaxis_title='p-value',
            height=400
        )
...
@st.cache_data(ttl=3600)
def cached_plot_differencing_comparison(original_series, differenced_series, title="Differencing Comparison"):
    """차분 비교 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
    return viz.plot_differencing_comparison(original_series, differenced_series, title=title)

@st.cache_data(ttl=3600)
def cached_plot_stationarity_comparison(adf_result, kpss_result, target_name="Time Series Data"):
    """정상성 비교 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
    return viz.plot_stationarity_comparison(adf_result, kpss_result, target_name=target_name)

@st.cache_data(ttl=3600)
def cached_plot_change_points(series, change_points_result, title="Change Point Analysis"):
    """구조적 변화점 분석 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
    return viz.plot_change_points(series, change_points_result, title=title)

@st.cache_data(ttl=3600)
def cached_plot_segment_means(series, change_points_result, title="Segment Means"):
    """세그먼트별 평균값 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
    return viz.plot_segment_means(series, change_points_result, title=title)

@st.cache_data(ttl=3600)
def cached_plot_correlation_heatmap(correlation_matrix, title="Correlation Heatmap"):
    """상관관계 히트맵 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
...
@st.cache_data(ttl=3600)
def cached_plot_residual_acf(residuals, max_lags=20, title="Residual Autocorrelation Function (ACF)"):
    """잔차 ACF 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
    return viz.plot_residual_acf(residuals, max_lags=max_lags, title=title)

@st.cache_data(ttl=3600)
def cached_plot_volatility(series, volatility, title="Time Series Data and Volatility"):
    """변동성 그래프 캐싱"""
    viz = TimeSeriesVisualizer()
