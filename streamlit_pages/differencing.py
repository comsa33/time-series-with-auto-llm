...
with st.form(key="differencing_form"):
    ...
    use_differencing = st.checkbox(
        "모델 학습에 차분 데이터 사용",
        value=False,
        help="차분된 데이터로 모델을 학습하고 예측합니다. 예측 결과는 자동으로 원래 스케일로 변환됩니다."
    )
...
with st.expander("차분 데이터 모델링 가이드", expanded=False):
    ...
    #### 3. LSTM 모델
    - LSTM은
     차분 데이터에도 잘 작동하며, 특히 추세와 계절성이 강한 경우 도움이 됩니다.
    
    #### 4. Prophet 모델
    - Prophet은 내부적으로 추세와 계절성을 모델링하므로, 일반적으로 원본 데이터 사용이 권장됩니다.
    
    > **참고**: 모델 학습 및 예측 페이지에서 차분 데이터로 모델을 학습하면, 예측 결과는 자동으로 원래 스케일로 변환됩니다.
    """)
